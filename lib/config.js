...
function buildDefaults() {
  ...
  // Local dev convenience: UI often served on :9011 but gateway runs on :18789.
  // If we're on localhost and *not* already on 18789, default to gateway :18789.
  const isLocal = hostname === 'localhost' || hostname === '127.0.0.1';
  const gatewayPortPart = (isLocal && port !== '18789') ? ':18789' : portPart;

  return {
    gatewayWsUrl: `${wsProtocol}//${hostname}${gatewayPortPart}/`,
    gatewayHttpUrl: `${protocol}//${hostname}${gatewayPortPart}`
  };
}
